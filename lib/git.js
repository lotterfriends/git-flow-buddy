    ...
    if (!branchName) {
      _getReleaseBranchName().then(function(branchName) {
        exec('git tag -d ' + branchName, function(error, stdout, stderr) {
          if (!noRemote) {
            exec(`git push origin :refs/tags/${branchName} --no-verify`, function(error, stdout, stderr) {
              resolve();
            });
          } else {
            resolve();
    ...
    if (!branchName) {
      _getReleaseBranchName().then(function(branchName) {
        ...
        if (!branchName) {
          ...
          exec(`git push origin --delete release/${branchName} --no-verify`, function(error, stdout, stderr) {
            if (error != null) {
              resolve();
            } else {
              resolve();
...
var _releaseStart = function(branchName) {
  return when.promise(function(resolve, reject) {
...
var _failSafeCommitChanges = function(message, filesToCommit) {
  return when.promise(function(resolve, reject) {
    exec('git add ' + filesToCommit.join(' '), function(error, stdout, stderr) {
      if (error != null) {
        reject(error);
      } else {
        exec(`git diff --quiet && git diff --staged --quiet || git commit --message="${message}"`, function(error, stdout, stderr) {
          if (error != null) {
            reject(error);
          } else {
            resolve();
...
var _isFileChanged = function(file) {
  return when.promise(function(resolve) {
    execSync(`git diff --exit-code -s ${file}`).on('exit', code => resolve(code === 1));
  });
};
...
var _fastForwardMaster = function() {
  return when.promise(function(resolve, reject) {
    exec(`git merge -s ours master -m "prepare branch"`, function(error, _stdout, _stderr) {
      if (error != null) {
        reject(error);
      } else {
        resolve();
...
var _finishRelease = function(push, keep, branchName, message) {
  return when.promise(function(resolve, reject) {
    ...
    exec('git flow release finish ' + doPush + doKeep + branchName, {maxBuffer: 1024 * 50000}, function(error, stdout, stderr) {
...
var _checkoutBranch = function(branch) {
  return when.promise(function(resolve, reject) {
    exec('git checkout -f ' + branch, function(error, stdout, stderr) {
      if (process.env.DEBUG_BRANCH_CHANGE && process.env.DEBUG_BRANCH_CHANGE === 'true') {
        console.log(stdout);
        console.log(stderr);
      }
      if (error != null) {
...
var _push = function() {
  return when.promise(function(resolve, reject) {
    exec('git push', function(error, _stdout, _stderr) {
      if (error != null) {
        reject(error);
      } else {
        resolve();
...
var _branchExistSync = function(branch) {
  try {
    execSync('git rev-parse --verify ' + branch, {stdio:['pipe', 'pipe', 'ignore']});
    return true;
  } catch (exception) {
    return false;
  }
...
var _getUser = function() {
  return when.promise(function(resolve, reject) {
    exec('git config user.name', function(errorUsername, stdoutUsername, _) {
...
var _getVersionPrefix = function() {
  ...
  return when.promise(function(resolve, reject) {
    exec('git config --get gitflow.prefix.versiontag', function(errorVersionT, versionTag, _) {
      if (errorVersionT != null) {
        reject('git config --get gitflow.prefix.versiontag failed');
      } else {
        resolve(versionTag.trim());
      }
    });
  });
}
...
module.exports = {
    ...
    isFileChanged: _isFileChanged,
    branchExist: _branchExist,
    branchExistSync: _branchExistSync,
    failSafeCommitChanges: _failSafeCommitChanges,
    pull: _pull,
    push: _push,
    status: _status,
    fastForwardMaster: _fastForwardMaster
}
