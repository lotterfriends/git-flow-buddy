...
var _finishRelease = function(push, keep, branchName, message) {
  return when.promise(function(resolve, reject) {
    ...
    execSync(`echo -e ${message} > .git/MY_TAGMSG`, {stdio:['pipe', 'pipe', 'ignore']});
    execSync('git config core.editor "mv .git/MY_TAGMSG"', {stdio:['pipe', 'pipe', 'ignore']});

    exec('git flow release finish ' + doPush + doKeep + ' -m "' + message + '" ' + branchName, function(error, stdout, stderr) {

      execSync('git config --unset core.editor"', {stdio:['pipe', 'pipe', 'ignore']});
      execSync('rm .git/MY_TAGMSG', {stdio:['pipe', 'pipe', 'ignore']});
