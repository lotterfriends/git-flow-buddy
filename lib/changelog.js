...
var _getUser = function(data) {
  return when.promise(function(resolve, reject) {
    if (!data.changelogUsername || data.changelogUsername === 'auto') {
      const releaseUser = process.env.gfb_release_user;
      if (releaseUser && releaseUser.length) {
        resolve(releaseUser);
      }
      git.getUser().then(username => {
        if (username && username.length) {
          resolve(username);
        } else {
          resolve(require("os").userInfo().username);
        }
      }, () => {
        resolve(require("os").userInfo().username);
      });
    } else if (data.changelogUsername === 'os') {
      resolve(require("os").userInfo().username);
    } else if (data.changelogUsername === 'git') {
      git.getUser().then(resolve, reject);
    } else if (data.changelogUsername === 'env') {
      resolve(process.env.gfb_release_user);
    } else {
      resolve(require("os").userInfo().username);
    }
  });
...
var _createChangelog = function(data) {
  ...
  return when.promise(function(resolve, reject) {
    ...
    _getUser(data).then(function(username) {
