...
function Release(options) {
  this.options = {
    ...
    postReleaseFinishCommands: options.postReleaseFinishCommands,
...
Release.prototype.runCommands = function(commands) {
  ...
  return when.promise(function(resolve, reject) {
    if (process.env.GFB_DISABLE_COMMANDS && process.env.GFB_DISABLE_COMMANDS === 'true') {
      return resolve();
    }
    ...
    commands.forEach(function(command) {
      commandPipe.push(function() {
        return when.promise(function(resolve, reject) {
          exec(command, {maxBuffer: 1024 * 2000}, function(error, stdout, stderr) {
            if (error != null) {
              console.log(stdout);
              if (stderr) {
                console.log(stderr);
...
Release.prototype.runpostReleaseFinishCommands = function() {
  if (this.options.debug) {
    console.log('Release.runpostReleaseFinishCommands()', this.options.postReleaseFinishCommands);
  }
  return this.runCommands(this.options.postReleaseFinishCommands);
};
...
Release.prototype.fastForwardMaster = function() {
  if (this.options.debug) {
    console.log('Release.fastForwardMaster()');
  }
  return git.fastForwardMaster();
};
...
Release.prototype.finish = function() {
  ...
  if (process.env.GFB_FF_MASTER && process.env.GFB_FF_MASTER === 'true') {
    releaseStack.push(this.fastForwardMaster.bind(this));
  }
  releaseStack.push(this.finishRelease.bind(this));

  if (_this.options.postReleaseFinishCommands.length) {
    releaseStack.push(_this.runpostReleaseFinishCommands.bind(_this));
  }
